...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (posterBtn) posterBtn.addEventListener('click', drawIQPoster);
...
function drawIQPoster() {
  try {
    ...
    if (!canvas) { showToast('⚠️ 海报生成失败'); return; }
    ...
    const iqText = els.iqScore ? els.iqScore.textContent || '100' : '100';
    const iq = parseInt(iqText) || 100;
    const label = els.iqLabel ? els.iqLabel.textContent || '' : '';
    const ctx = canvas.getContext('2d');
    if (!ctx) { showToast('⚠️ 浏览器不支持'); canvas.style.display = 'none'; return; }
    ...
    let msg = '';
    if (iq >= 130) msg = '大脑超频中，请勿轻易挑战 \u{1F680}';
    else if (iq >= 120) msg = '逻辑能力在线，已经超越了大部分人 \u{1F4AA}';
    else if (iq >= 110) msg = '脑子转得挺快，继续保持 \u{1F9E0}';
    else if (iq >= 90) msg = '大脑运行正常，状态不错 \u{1F60A}';
    else msg = '今天可能不在状态，改天再试试 \u{1F605}';
